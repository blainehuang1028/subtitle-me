...
The comparison section is one focused proofreading surface rather than a run of article cards. Tone, action, and rule-boundary cases remain visible together in three editorial columns; each keeps source, before/after copy, and the reason in one reading path. Stack the complete cases on mobile so no example depends on interaction or timing.
